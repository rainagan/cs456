...
//			System.out.println("sequence number = " + seqNum);
			...
			if (type == 0) {
				...
				// if ack packet has been dropped
				if (seqNum < expectSeqNum) {
					expectSeqNum = seqNum;
				} else {
					// save data into file
					file.print(new String(pack.getPayload()).trim());
				}
			...
			// if receive eot packet
				...
				log = "PKT RECV EOT ";
				log += (int) pack.getLength();
				log += " ";
				log += (int) pack.getSeqNum();
				System.out.println(log);
				
				file.print(new String("\n"));
				
				// send eot packet back and close
